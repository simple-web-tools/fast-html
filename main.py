...
def create_argparser_and_get_args():
    parser = argparse.ArgumentParser(
        prog="fast-html",
        description="Create html content fast. Note that this program will not overwrite your files, it simply generates valid html from short form html",
        epilog="visit www.cuppajoeman.com for more information",
    )

    parser.add_argument(
        "--base-dir",
        help="the base directory which fast-html will recursively operate on, path is relative to the fast-html directory",
    )
    parser.add_argument(
        "--gen-dir",
        help="the directory that fast-html will output the modified files, path is relative to the fast-html directory",
    )
    parser.add_argument(
        "--base-template-file",
        help="the base template file to be used on all short-form html files by default",
    )
    parser.add_argument(
        "--config-file",
        help="a fast-html config file, can be used to store configuraiton to speed up usage.",
    )
    parser.add_argument(
        "--custom-template-conversion-file",
        help="a python file which creates a variable with the name 'template_file_to_conversion' which is a dictionary mapping the name of a template file to a function which replaces a short-form html file with a valid html file. Check the readme for a specific example of this.",
    )
    parser.add_argument(
        "-d",
        "--devel",
        action="store_true",
        help="developer mode where only the changed files are built again",
    )
...
def convert_content_to_valid_html(
    ...
    if custom_conversion_module:
        ...
        assert (
            template_file_name in conversion_dict
        ), f"the template {template_file_name} doesn't have an entry in the conversion dictionary, the dictionary contains the following keys: {conversion_dict.keys()}"
        ...
        title_line_index = next(
            i for i, s in enumerate(template_lines) if "<title>" in s
        )
        ...
        line_index_right_after_body = (
            next(i for i, s in enumerate(template_lines) if "<body>" in s) + 2
        )
...
def copy_specific_files_to_the_generated_directory(
    content_directory_file_paths, content_directory, generated_directory
):
...
def convert_all_content_files_to_valid_html(
    ...
    for dir_path, dir_names, file_names in os.walk(generated_directory):
        ...
        for file_name in file_names:
            ...
            if any(
                normalized_full_path.endswith(os.path.normpath(ignore))
            ...
            if is_html_file:
                ...
                convert_content_to_valid_html(
                    full_path, file_name, base_template_file, custom_conversion_module
                )
                print(
                    f"~~~> converting {file_name} to valid html using {base_template_file}"
                )
...
def convert_specific_content_files_to_valid_html(
    files_to_convert: List[str], base_template_file: str, custom_conversion_module=None
):
...
def attempt_to_get_custom_conversion_module(custom_template_conversion_file: str):
    ...
    if custom_template_conversion_file is not None:
        ...
        spec = importlib.util.spec_from_file_location(
            module_name, custom_template_conversion_file
        )
...
def main():
    ...
    if args.config_file:
        ...
        custom_template_conversion_file = config["settings"].get(
            "custom-template-conversion-file"
        )
        ...
        ignored_files = (
            []
        )  # TODO: in the future might want to add cmd line support for this

    custom_conversion_module = attempt_to_get_custom_conversion_module(
        custom_template_conversion_file
    )

    if not base_dir or not gen_dir or not base_template_file:
        print(
            "Error: Missing required configuration (base-dir, gen-dir, base-template-file)."
        )
        return
    ...
    base_template_file = (
        base_template_file if base_template_file else "sample_template.html"
    )
    ...
    while True:
        ...
        modified_files = find_modified_files(
            base_dir_last_modified_times, base_dir_current_modified_times
        )
        ...
        if modified_files:
            ...
            copied_files = copy_specific_files_to_the_generated_directory(
                modified_files, base_dir, gen_dir
            )
            print(f"copied files {copied_files}")
            convert_specific_content_files_to_valid_html(
                copied_files, base_template_file, custom_conversion_module
            )
            save_mod_times_for_base_dir(base_dir)
        else:
